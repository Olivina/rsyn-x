...
void SolveByConjugateGradient( const Matrix &A, const vector< double > &b, vector< double > &x, const int maxNumIterations = 500, const double tolerance = 1e-5 ) {
	...
	if ( normf < 2.2204e-016 ) {
		cerr << "norm(f) is very close to zero, taking absolute residuum... as break condition.\n";
		normf = 1;
	} // end if
...
template<typename Matrix>
void SolveByConjugateGradientWithPCond( const Matrix &A, const vector< double > &b, vector< double > &x, const int maxNumIterations, const double tolerance, const SquareCompressedRowMatrix &U, const SquareCompressedRowMatrix &L) {
	vector<double> r(x.size()); // residual
	vector<double> d(x.size());
	vector<double> v(x.size()); // auxiliary vector to perform b - Ax
	vector<double> q(x.size());
...
template<typename Matrix> 
void SolveByConjugateGradientWithDiagonalPreConditioner( const Matrix &A, const vector< double > &b, vector< double > &x, const int maxNumIterations = 500, const double tolerance = 1e-5) {
	vector<double> r(x.size()); // residual
	vector<double> d(x.size());
	vector<double> v(x.size()); // auxiliary vector to perform b - Ax
	vector<double> q(x.size());
