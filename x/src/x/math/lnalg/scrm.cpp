...
void SparseMatrixDescriptor::AddElement( const int row, const int col, const double value ) {
	if ( col < 0 )
		throw SCRMException("Negative col index.");
	if ( row < 0 )
		throw SCRMException("Negative row index.");

	if ( row >= clsRows.size() )
		clsRows.resize( row + 1 );
...
void SparseMatrixDescriptor::AddDiagonalElement( const int row, const double value ) {
...
bool SparseMatrixDescriptor::CheckIsNumber( int & row, int & col ) const {
	...
	for ( row = 0; row < numRows; row++ ) {
		...
		for ( ColMap::const_iterator itcol = m.begin(); itcol != m.end(); itcol++ ) {
			const double val0 = itcol->second;
			const double val1 = ElementAt( itcol->first, row );
			
			if( std::isnan( val0 ) || std::isinf( val0 ) )
				return false;
...
void SparseMatrixDescriptor::Print( ostream &out ) const {
	...
	for ( int row = 0; row < numRows; row++ ) {
		const ColMap &m = clsRows[row];
		for ( ColMap::const_iterator itcol = m.begin(); itcol != m.end(); itcol++ ) {
			out << row << " " << itcol->first << " " << itcol->second << "\n";
		} // end for
	} // end for
} // end method
...
void UpperSparseMatrixDescriptor::AddElement( const int row, const int col, const double value ) {
	if ( col < row )
		throw SCRMException("Trying to set a lower element in an upper triangular matrix.");
...
void LowerSparseMatrixDescriptor::AddElement( const int row, const int col, const double value ) {
	if ( col > row )
		throw SCRMException("Trying to set an upper element in a lower triangular matrix.");
...
void AbstractSquareCompressedRowMatrix::Consolidate( const SparseMatrixDescriptor &dscp, const ConsolidationMode mode ) {
	const int numRows = dscp.GetNumRow();
	const int numCols = dscp.GetNumCol();

	if ( numRows != numCols )
		throw SCRMException("Non square matrix.");
	...
	clsRow.resize( dimension + 1 ); // + 1 adds a dummy row allowing to get the size of the row n using clsRow[n+1] - clsRow[n]
			...
			if ( row != col ) {
				switch (mode) {
					...
					case CONSOLIDATION_MODE_DISCARD_UPPER:
						...
						break;
					default:
						throw SCRMException( "Unknown consolidation mode.");
				} // end switch
	...
	// Since we may discard elements when building the crm matrix, the number
...
bool AbstractSquareCompressedRowMatrix::HasNullDiagonalElement(int &row, const double epsilon) const {
	const int numRows = GetDimension();
	for (row = 0; row < numRows; row++) {
		if (std::abs(clsDig[row]) <= epsilon)
			break;
	} // end for
	return row < numRows;
} // end method
...
bool AbstractSquareCompressedRowMatrix::HasEmptyRow(int &row) const {
	const int numRows = GetDimension();
	for (row = 0; row < numRows; row++) {
		const int numOffDiagonalElements = clsRow[row + 1] - clsRow[row];
		if (numOffDiagonalElements == 0)
			break;
	} // end for
	return row < numRows;
} // end method
...
void SquareCompressedRowMatrix::Initialize( const SparseMatrixDescriptor &dscp, const bool checkSymmetry ) {
	int symmetryFailCol;
	int symmetryFailRow;
	if ( checkSymmetry && !dscp.CheckSymmetry( symmetryFailRow, symmetryFailCol ) ) {
...
void SquareCompressedRowMatrix::Print( ostream &out ) const {
	...
	out <<  "Ptr = [ ";
	for ( size_t i = 0; i < clsRow.size(); i++ )
		out << clsRow[ i ] << "\n";
	out << "]\n";
	...
	for ( k = 0; k < N; k++) {
		...
		for (int i = clsRow[k]; i < clsRow[k+1]; i++) {
			...
			for (int j = clsRow[h] ; j < clsRow[h+1]; j++)
				for ( ; g < clsRow[k+1] && clsOff[g+1].col <= clsOff[j].col; g++)
					if (clsOff[g].col == clsOff[j].col)
						clsOff[j].val -= z * clsOff[g].val;
		} // end for
	} // end for
...
void SymmetricCompressedRowMatrix::Initialize( const SparseMatrixDescriptor &dscp, const bool checkMatrix ) {
	// Check symmetry.
	int symmetryFailCol;
	int symmetryFailRow;
	if ( checkMatrix ) {
		...
		if ( !dscp.CheckIsNumber( symmetryFailRow, symmetryFailCol ) )
			throw SCRMException( "The value at (" +
		...
		if ( !dscp.CheckSymmetry( symmetryFailRow, symmetryFailCol ) )
			throw SCRMException( "Non symmetric matrix at (" +
			std::to_string( symmetryFailRow ) + "," +
			std::to_string( symmetryFailCol ) + ")." );
	} // end if
...
void SymmetricCompressedRowMatrix::Initialize( const UpperSparseMatrixDescriptor &dscp ) {
	Consolidate( dscp, CONSOLIDATION_MODE_DISCARD_NONE);
} // end method
...
void SymmetricCompressedRowMatrix::CopyUpper( SquareCompressedRowMatrix &M ) const {
	M.clsOff = clsOff;
	M.clsRow = clsRow;
	M.clsDig = clsDig;
} // end method
...
void SparseMatrixBlock::fill_arrays (vector <vector <Element> > &matr, int NumRows) {
    ...
    if ((NumRows%num_block)== 0) aux_subd_mtr = (NumRows/num_block);
    else aux_subd_mtr = (NumRows/num_block) +1;

    for (int k=0; k<aux_subd_mtr; ++k) {
        for (int j=0; j<NumRows; ++j) {
            i=0;
            for (;matr[j][i+ aux_lin_subd[j]].row < aux_MAX_CACHE_VETX &&(matr[j].size() > i+ aux_lin_subd[j]); ++i) {
                matrAF[l].col = matr[j][i+ aux_lin_subd[j]].col;
                matrAF[l].row = matr[j][i+ aux_lin_subd[j]].row;
                matrAF[l].value = matr[j][i+ aux_lin_subd[j]].value;
                ++l;
            }
            aux_lin_subd[j] += i;
         }
